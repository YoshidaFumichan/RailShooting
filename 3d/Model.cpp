...
void Model::SetAmbient(const XMFLOAT3& ambient)
{
	for (auto m : materials)
	{
		m.second->ambient = ambient;
		m.second->Update();
	}
}

void Model::SetDiffuse(const XMFLOAT3& diffuse)
{
	for (auto m : materials)
	{
		m.second->diffuse = diffuse;
		m.second->Update();
	}
}

void Model::SetSpecular(const XMFLOAT3& specular)
{
	for (auto m : materials)
	{
		m.second->specular = specular;
		m.second->Update();
	}
}
